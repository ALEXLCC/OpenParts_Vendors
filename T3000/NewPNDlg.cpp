...
void CNewPNDlg::OnBnClickedOk()
{
    ...
    for (int i=0; i<table_pn.m_dataset_UN.size(); i++)
    {
        if (m_strUnits.CompareNoCase(table_pn.m_dataset_UN.at(i).UNUseUnits)==0)
        {
            m_item.PNUNID = table_pn.m_dataset_UN.at(i).UNID;
        }
    }
    ...
    if (!m_image_path.IsEmpty())
    {
        ...
        if (hFind!=INVALID_HANDLE_VALUE)//说明当前目录下无t3000.mdb
        {
            ...
            {
              stroldfilename =  table_pn.GetImageFileFold () +m_item.PNImageName; 
            }
            strSuffix.Format (_T(".%s"), m_image_path.Right (3));
            m_item.PNImageName = NewGUID () + strSuffix;
            CString strDest;
            strDest=  table_pn.GetImageFileFold () +m_item.PNImageName;
...
BOOL CNewPNDlg::OnInitDialog()
{
    CDialogEx::OnInitDialog();
     ...
     table_pn.Initial_Type();
     table_pn.Initial_UN();
     table_pn.Initial_Data ();
     for (int i=0; i<table_pn.m_dataset_type.size(); i++)
     {
         m_combox_type.AddString(table_pn.m_dataset_type.at(i).TYPEType);
     }
     for (int i=0; i<table_pn.m_dataset_UN.size(); i++)
     {
         m_combox_unit.AddString(table_pn.m_dataset_UN.at(i).UNUseUnits);
     }
...
void CNewPNDlg::LoadData()
{
    if (m_new_item==1||m_new_item == 2)
    {
        ...
        m_strUnits=table_pn.GetUnitName_ByID(m_item.PNUNID);
        ...
        if (m_item.PNImageName.IsEmpty ())
        {
            m_imagefile = table_pn.GetImageFileFold ()+L"default.jpg";
        }
        else
        {
            m_imagefile = table_pn.GetImageFileFold ()+m_item.PNImageName;
        }
